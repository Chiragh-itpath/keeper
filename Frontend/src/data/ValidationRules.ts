...
const emailRules = (value: any) => (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(value) ? true : 'E-mail must be valid.')
const passwordRule = (val: string) => (val.length < 8 ? 'At least 8 characters!' : true)
const contactRules = (value: string) =>
...
const otpRules = (value: string) =>
  /^[0-9]{6}$/.test(value) || value == '' ? true : 'OTP must be valid'
const numberRules = (value: string) =>
  /^[0-9]{5}$/.test(value) || value == '' ? true : 'Number must be valid'
export {
  requiredRule,
  emailRules,
  passwordRule,
